...
func (c *Configuration) Route(r *RouteBuilder) *Configuration {
	...
	if _, exists := c.routes[r.version]; !exists {
		c.routes[r.version] = make(controllers)
	}
	if _, exists := c.routes[r.version][fullResource]; !exists {
		c.routes[r.version][fullResource] = make(actions)
	}
